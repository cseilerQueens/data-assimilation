...
colnames(all) <- PFTs
  ...
  my.ylab <- latex2exp::TeX("$v_{cmax}$ ($\\mu$mol CO$_2$ m$^{-2}$ s$^{-1}$)")
  
  png("syntheticDataTest.png", width = 5, height = 4, units = "in", res = 300)
  par(mar = c(8,5,1,1))
  ...
  legend("topleft", pch = c(17, 16, 1), col = c("blue", "red", "black"), 
         c("Default", "Optimized", "Range"), bty = "n", horiz = TRUE, cex = 0.75)
